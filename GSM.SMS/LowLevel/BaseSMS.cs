...
namespace GSM.Messages
{
	...
	public abstract class BaseSMS
	{
        ...
        public static string Decode7bit(string source, int length)
		{
			...
			List<Byte> resultGSM = new List<Byte>(140);

			for (int i = 1; i <= length; i++)
				resultGSM.Add(Convert.ToByte(binary.Substring(binary.Length - i * 7, 7), 2));

            return Gsm7String.ToUnicode(resultGSM);
		}
		...
		public static string PopAddress(ref string source, int length)
		{
            ...
            if (addressType != (byte)GSM.NumberFormat.alphanumFormat)
            {
                address = ReverseBits(address).Trim('F');

                if (addressType == (byte)GSM.NumberFormat.internationalFormat)
                    address = "+" + address;
            }
            else
            {
                address = Decode7bit(address, (address.Length/2) * 8 / 7);
            }
		...
		public static void Fetch(BaseSMS sms, ref string source)
		{
