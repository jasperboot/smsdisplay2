...
{
	public class TextMessage : BaseSMS
	{
		#region Members
        ...
        private int InPartsType
        {
            get
            {
                ...
                if (_userDataHeader[0] == 0x00 && _userDataHeader[1] == 0x03) return 1; //  8-bit referenceno (default)
                if (_userDataHeader[0] == 0x08 && _userDataHeader[1] == 0x04) return 2; // 16-bit referenceno
                return 0;
            }
        }
        ...
        public string MultiPartID
        {
            get
            {
                if (!this.InParts) return null;
                return string.Format("{0}{1}", this.InPartsID, this.PhoneNumber);
            }
        }
		...
		public int InPartsID
		{
			get
			{
                switch (InPartsType)
                {
                    ...
                    case 2:
                        return ((_userDataHeader[3] << 8) + _userDataHeader[2]);
                    default:
                        return 0;
                }
			}
        ...
        // Jasper: Toegevoegd volgens spec op Wikipedia
		...
		public override SMSType Type { get { return SMSType.Normal; } }
		#endregion

		#region Public Statics
		public static void Fetch(TextMessage sms, ref string source)
		{
			BaseSMS.Fetch(sms, ref source);
            ...
            byte userDataHeaderLength = 0;
			if (sms._userDataStartsWithHeader) {
				userDataHeaderLength = PeekByte(source);
				
				sms._userDataHeader = PeekBytes(source, 1, userDataHeaderLength);
//                userDataLength -= (userDataHeaderLength + 1);
            ...
            // Jasper: UDH-tekens niet vergeten te verwijderen... Gewoon de userDataLength knippen, zoals oorspronkelijk bedoeld werkt even niet.
            int headerStripCount=0;
			switch ((SMSEncoding) sms._dataCodingScheme & SMSEncoding.ReservedMask) {
				case SMSEncoding._7bit:
                    if (userDataHeaderLength > 0) headerStripCount = (int)Math.Ceiling((userDataHeaderLength + 1) * 8 / 7.0);
                    sms._message = Decode7bit(source, userDataLength);
					break;
				case SMSEncoding._8bit:
                    if (userDataHeaderLength > 0) headerStripCount = userDataHeaderLength + 1;
            ...
            if (headerStripCount > 0) sms._message = sms._message.Substring(headerStripCount);
        }
